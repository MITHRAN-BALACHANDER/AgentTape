...
```toml
[redact]
# HTTP Headers to scrub completely
headers = ["Authorization", "X-Api-Key", "Cookie"]

# Specific string values to search and replace everywhere
secrets = ["sk-proj-12345", "super_secret_password"]

# The string to replace the sensitive data with (default: <REDACTED>)
...
```toml
[redact]
# Read the value from the environment variable, and redact that value
...
```bash
# Scrub the string "john@example.com" from the cassette
